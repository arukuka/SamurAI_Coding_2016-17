...
class DQN():
    ...
    def get_loss(self, rules):
        ...
        print >> sys.stderr, "\tacc0 : {}".format(float(F.accuracy(Q0, Variable(target0)).data))
        print >> sys.stderr, "\tacc1 : {}".format(float(F.accuracy(Q1, Variable(target1)).data))
        print >> sys.stderr, "\tacc2 : {}".format(float(F.accuracy(Q2, Variable(target2)).data))
        return F.softmax_cross_entropy(Q0, Variable(target0)), F.softmax_cross_entropy(Q1, Variable(target1)), F.softmax_cross_entropy(Q2, Variable(target2))
